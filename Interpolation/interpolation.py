...
def CSInterpolation(filepath, smp):
    ...
    CpCS = np.diff(interpolated_values[:, 1]) / np.diff(
        interpolated_values[:, 0]
    )  # Forward Difference
    CpBC = np.diff(interpolated_values[:, 1][::-1]) / np.diff(
        interpolated_values[:, 0][::-1]
    )  # Backward Difference
    CpCD = np.zeros_like(cx)  # Initialize array for Central Difference
    for i in range(1, len(cx) - 1):
        CpCD[i] = (cy[i + 1] - cy[i - 1]) / (
            2 * (cx[i + 1] - cx[i])
        )  # Central Difference
    ...
    ax[1].plot(
        ...
        lw=3,
        alpha=0.7,
        label="Cubic Spline (Forward Diff)",
    )
    ax[1].plot(
        interpolated_values[:-1, 0][::-1],
        CpBC,
        "g--",
        lw=3,
        alpha=0.7,
        label="Cubic Spline (Backward Diff)",
    )
    ax[1].plot(
        cx[1:-1],
        CpCD[1:-1],
        ...
        lw=3,
        alpha=0.7,
        label="Cubic Spline (Central Diff)",
    )
    ax[1].legend()
